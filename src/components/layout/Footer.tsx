...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-[var(--color-background-secondary,#f7f5f3)] text-[var(--color-text-primary,#1a1a1a)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr] gap-8 pb-6 border-b border-black/10">

          {/* BRAND COLUMN */}
          <div>
            <Link to="/" className="inline-block mb-3">
              <h2 className="heading-serif text-lg font-medium tracking-wide">
                {BRAND.nameTop}
            ...
            <div className="flex items-center gap-3">
              {(CONTACT.instagram ?? SITE.instagram) && (
                <a
                  ...
                  className="text-black/50 hover:text-rose-gold-dark transition-colors"
                >
              ...
              {(CONTACT.whatsapp ?? CONTACT.phone) && (
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label="WhatsApp"
                  className="text-black/50 hover:text-green-600 transition-colors"
                >
          ...
          <div>
            <p className="text-sm font-medium text-black/60 mb-3">Shop</p>
            <ul className="space-y-2.5">
                ...
                categories.map(category => (
                  <li key={category.id}>
                    <Link
                      ...
                      className="text-sm text-black/60 hover:text-rose-gold-dark transition-colors"
                    >
                      {category.name}
                    </Link>
                  </li>
              ...
              <li>
                <Link to="/new-arrivals" className="text-sm text-black/60 hover:text-rose-gold-dark transition-colors">
                  New Arrivals
                </Link>
              </li>
              <li>
                <Link to="/sale" className="text-sm text-black/60 hover:text-rose-gold-dark transition-colors">
                  Sale
                </Link>
              </li>
              <li>
                <Link to="/wishlist" className="text-sm text-black/60 hover:text-rose-gold-dark transition-colors">
                  Wishlist
                </Link>
              </li>
            </ul>
          ...
          {/* HELP COLUMN */}
          <div>
            <p className="text-sm font-medium text-black/60 mb-3">Help</p>
            <ul className="space-y-2.5">
              {helpLinks.map(link => (
                <li key={link.to}>
                  <Link
          ...
          <div>
            <p className="text-sm font-medium text-black/60 mb-3">Stay in touch</p>
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex gap-1.5">
              <input
                ...
                className="flex-1 text-sm px-3 py-2 rounded-md border border-black/15 bg-white focus:outline-none focus:ring-1 focus:ring-rose-gold-dark"
              />
              <button
                type="submit"
                aria-label="Subscribe"
                className="px-3 rounded-md bg-black/90 text-white hover:bg-rose-gold-dark transition-colors"
              >
                <ArrowRight size={16} />
              </button>
            </form>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-4">
          ...
          <div className="flex gap-4">
            {legalLinks.map(link => (
              <Link
                key={link.to}
                to={link.to}
                className="text-xs text-black/40 hover:text-rose-gold-dark transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex gap-2 flex-wrap">
            {(SITE.paymentMethods ?? ['bKash', 'Nagad', 'COD']).map(method => (
              <span
                key={method}
                className="px-3 py-1 bg-black/5 rounded-md text-xs text-black/50"
              >
                {method}
              </span>
            ))}
